...
// Derived, not stored: expand every vedtak in force into the occurrences it
// grants on this date, then let the reported visits settle which happened.
// Pure, so the edge cases are testable without a database.
public static class DayPlanBuilder
{
    ...
    // Chronological where a time is known; an unbooked occurrence has none, so
    // it sorts after the service's booked ones instead of landing arbitrarily among them.
    ...
    // Two vedtak can grant the same service on one day, so the occurrences are
    // pooled per service and numbered once across the day rather than per vedtak.
