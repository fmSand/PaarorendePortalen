...
// Stands in for a municipal EPJ. Access to one takes an agreement with the
// municipality, so this is the implementation that ships.
public sealed class SyntheticVisitSource : IVisitSource
{
    ...
    )
    {
        ...
        // Built per fetch, so a visit that has been and gone stops reporting itself as planned.
