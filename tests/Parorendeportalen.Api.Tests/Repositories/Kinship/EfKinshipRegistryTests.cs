...
namespace Parorendeportalen.Api.Tests.Repositories.Kinship;
...
public class EfKinshipRegistryTests(PostgresContainerFixture fixture) : IAsyncLifetime
{
    ...
    // An expired grant drops out and the person row stays, so login rejects on the empty grant set.
    [Fact]
    ...
    // Without these, dropping the ValidFrom half of the validity filter goes undetected,
    // and a future-dated grant would authorise immediately.
    [Fact]
    ...
    // The pair is unique. One person can hold several grants, one per care recipient.
    [Fact]
