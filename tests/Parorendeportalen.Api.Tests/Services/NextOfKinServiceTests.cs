...
public class NextOfKinServiceTests
{
    ...
    public async Task GetByExternalIdAsync_ReturnsMappedNextOfKin_WhenFound()
    {
        ...
        Assert.Equal(7, result.CareRecipientId);
    }
    ...
    [Fact]
    public async Task GetCareRecipientIdByExternalIdAsync_ReturnsId_WhenFound()
    {
        _registry.GetByExternalIdAsync("sub-123", Arg.Any<CancellationToken>())
            .Returns(new NextOfKin { Id = 1, ExternalId = "sub-123", NationalIdHash = "h", DisplayName = "N", CareRecipientId = 7 });

        var result = await _sut.GetCareRecipientIdByExternalIdAsync("sub-123", CancellationToken.None);

        Assert.Equal(7, result);
    }

    [Fact]
    public async Task GetCareRecipientIdByExternalIdAsync_ReturnsNull_WhenNotFound()
    {
        _registry.GetByExternalIdAsync("unknown-sub", Arg.Any<CancellationToken>())
            .Returns((NextOfKin?)null);

        var result = await _sut.GetCareRecipientIdByExternalIdAsync("unknown-sub", CancellationToken.None);
    ...
    public async Task ResolveOrBindAsync_BindsSeededGrant_OnFirstMatchByNationalIdHash()
    {
        ...
        Assert.Equal(1, result.CareRecipientId);
