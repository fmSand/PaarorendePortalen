...
{
    private readonly ICareRecipientRepository _repository =
    ...
    {
        _repository.GetByIdAsync(999, Arg.Any<CancellationToken>()).Returns((CareRecipient?)null);
    ...
    {
        _repository.GetByIdAsync(3, Arg.Any<CancellationToken>()).Returns((CareRecipient?)null);
        _repository
            .GetByIdAsync(Arg.Is<int>(id => id != 3), Arg.Any<CancellationToken>())
            .Returns(new CareRecipient { Id = 1, Name = "Kari Nordmann" });
