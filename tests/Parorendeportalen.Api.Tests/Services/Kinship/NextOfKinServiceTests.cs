...
namespace Parorendeportalen.Api.Tests.Services.Kinship;
...
public class NextOfKinServiceTests
{
    ...
    )
    {
        ...
        person.Grants.AddRange(
            careRecipientIds.Select(careRecipientId => new KinshipGrant
            {
                NextOfKinId = id,
                CareRecipientId = careRecipientId,
                CareRecipient = new CareRecipient
                {
                    Id = careRecipientId,
    ...
    public async Task GetCareRecipientIdsByExternalIdAsync_ReturnsEveryGrantedId()
    {
        ...
        var result = await _sut.GetCareRecipientIdsByExternalIdAsync(
            "sub-123",
    ...
    public async Task ResolveOrBindAsync_ReturnsExisting_WhenExternalIdAlreadyBound_AndDisplayNameUnchanged()
    {
        ...
        var result = await _sut.ResolveOrBindAsync(
            "sub-123",
            "12345678901",
            "Frida Sand",
            CancellationToken.None
        ...
        await _registry
            .DidNotReceive()
            .UpdateAsync(Arg.Any<NextOfKin>(), Arg.Any<CancellationToken>());
        await _registry
            .DidNotReceive()
            .GetByNationalIdHashAsync(Arg.Any<string>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task ResolveOrBindAsync_RefreshesDisplayName_WhenExternalIdBound_ButNameChanged()
    {
        ...
        await _registry
            .Received(1)
            .UpdateAsync(
                Arg.Is<NextOfKin>(n => n.DisplayName == "New Name" && n.Id == 1),
    ...
    {
        _registry
            .GetByExternalIdAsync("new-sub-456", Arg.Any<CancellationToken>())
            .Returns((NextOfKin?)null);

        var expectedHash = _hasher.Hash("12345678901");
        _registry
            .GetByNationalIdHashAsync(expectedHash, Arg.Any<CancellationToken>())
            .Returns(
                CreatePerson(
                    id: 3,
                    externalId: null,
                    nationalIdHash: expectedHash,
        ...
        await _registry
            .Received(1)
            .UpdateAsync(
                Arg.Is<NextOfKin>(n => n.ExternalId == "new-sub-456" && n.Id == 3),
                Arg.Any<CancellationToken>()
            );
    }
    ...
    {
        _registry
            .GetByExternalIdAsync("stranger-sub", Arg.Any<CancellationToken>())
            .Returns((NextOfKin?)null);
        _registry
            .GetByNationalIdHashAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns((NextOfKin?)null);
    ...
    // A person row can outlive its grants, and identity alone must not authorise.
    [Fact]
    public async Task ResolveOrBindAsync_ReturnsNull_WhenPersonExistsButHoldsNoCurrentGrant()
    {
        _registry
            .GetByExternalIdAsync("expired-sub", Arg.Any<CancellationToken>())
